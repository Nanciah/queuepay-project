...
// ========== STATUT DE LA FILE AVEC CRÉNEAUX ==========
router.get('/agent/queue-slot/:serviceId/status', auth, isAgent, async (req, res) => {
    try {
        const { serviceId } = req.params;
        const agentId = req.user.id;
        ...
        // Récupérer les tickets avec rendez-vous (appointment_date non null)
        const tickets = await Ticket.findAll({
            where: {
                service_id: serviceId,
                appointment_date: { [Op.not]: null },
                status: { [Op.in]: ['pending', 'waiting', 'called'] }
            },
            order: [
        ...
        // Tickets sans rendez-vous (file d'attente classique)
        ...
        // Statistiques
        const waitingSlots = tickets.filter(t => t.status === 'pending' || t.status === 'waiting').length;
        const calledSlots = tickets.filter(t => t.status === 'called').length;
        const waitingRegular = regularTickets.filter(t => t.status === 'pending' || t.status === 'waiting').length;

        // Ticket en cours
        const currentTicket = tickets.find(t => t.status === 'called') || 
                             regularTickets.find(t => t.status === 'called');
        ...
        res.json({
            ...
            stats: {
                ...
                waiting_slots: waitingSlots,
                called_slots: calledSlots,
                total_regular: regularTickets.length,
                waiting_regular: waitingRegular
            },
            currentTicket: currentTicket ? {
                id: currentTicket.id,
                ticket_number: currentTicket.ticket_number,
                status: currentTicket.status,
                appointment_date: currentTicket.appointment_date,
                appointment_time: currentTicket.appointment_time,
                client: currentTicket.client ? {
                    first_name: currentTicket.client.first_name,
                    last_name: currentTicket.client.last_name,
                    phone: currentTicket.client.phone
...
// ========== APPELER UN CLIENT AVEC CRÉNEAU ==========
router.post('/agent/queue-slot/:ticketId/call', auth, isAgent, async (req, res) => {
    try {
        const { ticketId } = req.params;
        const agentId = req.user.id;
        ...
        // Vérifier l'accès
        if (ticket.service.agent_id !== agentId) {
            return res.status(403).json({ error: 'Accès refusé' });
        }

        // ✅ Vérifier que le compte à rebours est terminé
        if (ticket.appointment_date && ticket.appointment_time) {
            const now = new Date();
            const appointmentDateTime = new Date(`${ticket.appointment_date}T${ticket.appointment_time}`);
            
            if (appointmentDateTime > now) {
                const diffMinutes = Math.floor((appointmentDateTime - now) / 60000);
                return res.status(400).json({
                    error: `Le rendez-vous est dans ${diffMinutes} minutes. Le bouton sera disponible à ${ticket.appointment_time}.`
                });
            }
        }
        ...
        try {
            const io = getIO();
            if (io) {
                io.to(`ticket:${ticket.id}`).emit('you-are-called', {
                    ticketId: ticket.id,
                    ticketNumber: ticket.ticket_number,
                    serviceId: ticket.service_id,
                ...
                console.log(`📡 you-are-called émis pour ticket ${ticket.ticket_number}`);
            }
        } catch (wsError) {
            console.error('❌ Erreur WebSocket:', wsError);
        }
...
router.get('/agent/queue-slot/:serviceId/status', auth, isAgent, async (req, res) => {
    try {
        ...
        console.log(`📊 [queue-slot] Récupération des rendez-vous pour service ${serviceId}`);
        ...
        // Récupérer les tickets avec rendez-vous
        const slots = await Ticket.findAll({
            where: {
                service_id: serviceId,
                appointment_date: { [Op.not]: null },
                status: { [Op.in]: ['pending', 'waiting', 'called'] }
        ...
        // Tickets sans rendez-vous
        const regular = await Ticket.findAll({
            where: {
                service_id: serviceId,
                appointment_date: null,
                status: { [Op.in]: ['pending', 'waiting', 'called'] }
        ...
        // Statistiques
        const waitingSlots = slots.filter(t => t.status === 'pending' || t.status === 'waiting').length;
        const calledSlots = slots.filter(t => t.status === 'called').length;
        const waitingRegular = regular.filter(t => t.status === 'pending' || t.status === 'waiting').length;

        // Ticket en cours
        const currentTicket = slots.find(t => t.status === 'called') || 
                             regular.find(t => t.status === 'called');
        ...
        console.error('❌ Erreur queue-slot:', error);
        res.status(500).json({ error: error.message });
    }
});

// ========== APPELER UN CLIENT AVEC RENDEZ-VOUS ==========
router.post('/agent/queue-slot/:ticketId/call', auth, isAgent, async (req, res) => {
    try {
        const { ticketId } = req.params;
        const agentId = req.user.id;

        console.log(`📞 [queue-slot] Appel du ticket ${ticketId}`);
        ...
        // ✅ Vérifier que le compte à rebours est terminé (pour les rendez-vous)
        if (ticket.appointment_date && ticket.appointment_time) {
            const now = new Date();
            const appointmentDateTime = new Date(`${ticket.appointment_date}T${ticket.appointment_time}`);
